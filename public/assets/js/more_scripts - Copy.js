...
function LoginUser(e) {
    ...
    return jQuery.ajax({
        ...
        success: function(e) {
            if (console.log("response data", e), "success" == e.status) eraseCookie("user_id"), eraseCookie("username"), eraseCookie("image"), eraseCookie("profile_link"), eraseCookie("onoff"), eraseCookie("email"), toastr.success(e.message), window.location.href = e.url;
...
$(document).ready(function() {
        ...
        jQuery(document).on("click", "#search-toggle", function(e) {
            ...
            if (e.preventDefault(), anchor = jQuery("#web_link_model_form form #anchor").val(), website = jQuery("#web_link_model_form form #website").val(), link = jQuery("#web_link_model_form form #link").val(), term = jQuery("#web_link_model_form form #term").val(), blog_id = jQuery("#web_link_model_form form #blog_id_x").val(), 0 == anchor.length) return 0;
            if (jQuery("#term").is(":checked")) {} else {
                return 0;
            }
            jQuery("#web_link_model_form").modal("hide"), jQuery("#web_link_model_after_submit").modal("show"), jQuery.ajax({
                type: "post",
...
1 === onlyMessageChatPage && (get_unread_users(), get_inbox_users(), setTimeout(function() {
    ...
    jQuery(".message-sidebar .nav-tabs a").on("click", function() {
        ...
        if (r.preventDefault(), message = e[0].emojioneArea.getText(), "" == message.trim() && (message = jQuery("#send-message-form .emojionearea-editor").text()), "" != message.trim() || 0 != jQuery("#message_file")[0].files.length) {
            ...
            var data_image = document.getElementById("message_file").value = "";
            t.append("message", message), t.append("currentUser", ""), btn = jQuery("#send-message-form button")
             if(jQuery('input[name="received_by"]').val().length > 0)  {
             	...
             	jQuery.ajax({
                             ...
                             success: function(r) {
                                 jQuery('.loding').hide();
                                 jQuery("#lmi").text(r.lmi), jQuery("#send-message-form textarea").val(""), e[0].emojioneArea.setText(""), jQuery("#load").css("visibility", "hidden"), jQuery("#message_file").val(""), jQuery(".direct-chat-messages").append(r.html), jQuery(".direct-chat-messages").scrollTop(jQuery(".direct-chat-messages")[0].scrollHeight)
                             },
                             error: function(r) {
                                 jQuery("#send-message-form textarea").val(""), e[0].emojioneArea.setText(""), toastr.error("Bad Request.")
                             }
                         })} else {(jQuery("#send-message-form textarea").val(""), e[0].emojioneArea.setText(""))}
        }

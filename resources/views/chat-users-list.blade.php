...
<ul class="contacts" id="contacts">
   ...
   <li class="active">
      ...
      @endif
      <a href="javascript:void(0);" class="chat-toggle" data-id="{{ $user->user_id }}" data-user="{{ $user->first_name.' '.$user->last_name ?: '' }}" data-imgsrc="{{ !empty($user->profile_pic) && file_exists($user->profile_pic) ? asset($user->profile_pic) : url('assets/images/star-logo.png') }}" data-plink="{{ $profile_link }}" data-onoff="{{ $on_off }}" data-email="{{ $user->email }}">
      <div class="d-flex bd-highlight">
         <div class="img_cont">
            <img src="{{ !empty($user->profile_pic) && file_exists($user->profile_pic) ? asset($user->profile_pic) :  url('assets/images/star-logo.png') }}" class="rounded-circle user_img" id="xxx" onerror="this.onerror=null;this.src='/assets/images/buyer/b-acount.png';">
            @if(Cache::has('user_is_online_' . $user->user_id))
            @php $on_off =  'online' @endphp
   ...
   <li class="active">
     ...
     <a href="javascript:void(0);"  class="chat-toggle" data-id="{{ $user->user_id }}" data-user="{{ $user->first_name.' '.$user->last_name ?: '' }}" data-imgsrc="{{ !empty($user->profile_pic) && file_exists($user->profile_pic) ? asset($user->profile_pic) : url('assets/images/star-logo.png') }}" data-plink="{{ $profile_link }}" data-onoff="{{ $on_off }}" data-email="{{ $user->email }}">
      <div class="d-flex bd-highlight">
         <input id="user_sender_id" type="hidden" value="{{ Auth::user()->id }}" name="">
         <div class="img_cont">
            <img src="{{ !empty($user->profile_pic) && file_exists($user->profile_pic) ? asset($user->profile_pic) :  url('assets/images/star-logo.png') }}" class="rounded-circle user_img" id="xxx" onerror="this.onerror=null;this.src='/assets/images/buyer/b-acount.png';">
            @if(Cache::has('user_is_online_' . $user->user_id))
            @php $on_off =  'online' @endphp

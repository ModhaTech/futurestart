    ...
    public function index(Request $request)
    {
        if(!empty($request['search'])){
            ...
            foreach($qry as $q){
             ...
             $sellers = User::where('username', 'like', '%' . $searchQuery . '%')->where($sellerCondition)->where('status','=',1)->get();
             $sellersCount = count($sellers);

             $buyerCondition = ['role_id' => 3];
             $buyers = User::where('username', 'like', '%' . $searchQuery . '%')->where($buyerCondition)->where('status','=',1)->get();

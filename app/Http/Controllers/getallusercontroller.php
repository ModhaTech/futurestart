...
class getallusercontroller extends Controller 
{
	...
	{
	 /**  true query for check user unread msg.*	SELECT u.id,u.first_name,u.last_name,m.sent_by,m.received_by,sum(case when m.read_flag = 0 then 1 else 0 end) as*  read_count FROM messages m INNER JOIN users u ON m.sent_by = u.id GROUP BY m.sent_by**/

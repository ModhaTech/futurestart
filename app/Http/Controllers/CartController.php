...
class CartController extends Controller
{
	...
	public function storeStripe(Request $request)
	{
		// return $request;
		$myArray = [];
		foreach ($request->selleres as $key => $value) {
			$myArray['seller_id'][] = $value['id']; 
            $myArray['seller_amount'][] = $value['amount']; 
		}
		...
		try {
			...
			$PaymentIntent = $stripe->paymentIntents->create([
		  	...
		  	if ($PaymentIntent->status == "succeeded") 
		  	{
				...
				foreach ($pps as $key => $pp) 
				{
					...
					// $bp->user_id	=	Talents::find($pp->talent_id)->first()->user_id;
					$bp->user_id	=	Talents::find($pp->talent_id)->user->id;
					$bp->buyer_id	=	Auth::id();
        	...
        	return redirect('/buyer/shopping-cart/');
